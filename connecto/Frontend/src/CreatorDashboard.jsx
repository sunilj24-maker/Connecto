    ...
    const fetchData = async () => {
      try {
        ...
        if (profileError && profileError.code !== "PGRST116") {
          ...
          // No profile found, open modal for new user
          setIsProfileModalOpen(true);
        }
      } catch (err) {
            ...
            {isLoading ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {[1, 2, 3, 4, 5, 6].map((i) => (
                  <div
                    key={i}
                    className="border border-slate-200 bg-white rounded-xl overflow-hidden animate-pulse shadow-sm"
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {sortedCampaigns.map((camp) => {
                  ...
                  const hasFollowers =
                    creatorProfile.follower_count >= minFollowers;
                  const hasAgeMatch = calculateAgeOverlap(
                    creatorProfile.age_min,
                    creatorProfile.age_max,
                    camp.age_min,
                    camp.age_max,
                  );
                  const isEligible = hasFollowers && hasAgeMatch;

                  const campInterests = Array.isArray(camp.interests)
                    ? camp.interests
                    : [];
                  const matchedInterests = campInterests.filter((i) =>
                    creatorProfile.interests.includes(i),
                  );

                  return (
                    <div
                      key={camp.id}
                      className="border border-slate-200 rounded-xl overflow-hidden bg-white flex flex-col group hover:border-slate-300 transition-colors shadow-sm hover:shadow"
                    >
                      ...
                      <div className="p-5 flex flex-col flex-1">
                        <h2 className="text-lg font-bold tracking-tight mb-1.5 leading-tight line-clamp-2">
                          {camp.title}
                        </h2>
                        ...
                        <div className="flex flex-wrap gap-2 mb-4">
                          {matchedInterests.length > 0 ? (
                            matchedInterests.slice(0, 2).map((i) => (
                              <span
                                key={i}
                                className="text-[11px] bg-slate-100 text-slate-600 px-2 py-1 rounded font-bold tracking-tight uppercase"
                              >
                                {i}
                              </span>
                            ))
                          ...
                          <span className="text-[11px] bg-indigo-50 text-indigo-600 px-2 py-1 rounded font-bold tracking-tight uppercase border border-indigo-100">
                            {minFollowers > 0
                              ? `${(minFollowers / 1000).toFixed(1)}k+ FLW`
                              : "Any FLW"}
                          </span>
                        </div>
                        ...
                        <div className="mt-auto pt-4 border-t border-slate-100 flex items-center justify-between">
                          ...
                          <div className="font-bold tracking-tight text-lg text-black">
                            {camp.payout_amount
                              ? `₹${camp.payout_amount.toLocaleString()}`
                              : "Variable"}
                          </div>
                        </div>
                        ...
                        <div className="mt-5">
                          {isEligible ? (
                            ...
                            <button
                              disabled
                              className="w-full bg-slate-100 text-slate-400 font-bold tracking-tight py-2.5 rounded-lg text-sm cursor-not-allowed border border-slate-200/60"
                            >
                              🔒 Criteria Not Met
                            </button>
                          )}
